...
if __name__ == "__main__":
    ...
    for id_no, dados in grafo.nos_requeridos.items():
        print(f"N{id_no}: Demanda={dados['demanda']}, Custo de Serviço={dados['custo_servico']}")

    print("\nARESTAS REQUERIDAS:")
    for aresta in grafo.arestas_requeridas:
        print(f"E: {aresta['de']} <-> {aresta['para']} | Transporte={aresta['custo_transporte']}, "
              f"Demanda={aresta['demanda']}, Serviço={aresta['custo_servico']}")

    print("\nARCOS REQUERIDOS:")
    for arco in grafo.arcos_requeridos:
        print(f"A: {arco['de']} -> {arco['para']} | Transporte={arco['custo_transporte']}, "
              f"Demanda={arco['demanda']}, Serviço={arco['custo_servico']}")

    print("\nARESTAS NÃO REQUERIDAS:")
    for aresta in grafo.arestas_nao_requeridas:
        print(f"NrE: {aresta['de']} <-> {aresta['para']} | Transporte={aresta['custo_transporte']}")

    print("\nARCOS NÃO REQUERIDOS:")
    for arco in grafo.arcos_nao_requeridos:
        print(f"NrA: {arco['de']} -> {arco['para']} | Transporte={arco['custo_transporte']}")

    print("\n--- Estatísticas do Grafo ---")
    print(f"Total de vértices: {grafo.total_vertices()}")
    print(f"Total de arestas: {grafo.total_arestas()}")
    print(f"Total de arcos: {grafo.total_arcos()}")
    print(f"Total de vértices requeridos: {grafo.total_vertices_requeridos()}")
    ...
    print(f"Densidade do grafo: {grafo.densidade():.4f}")
    print(f"Componentes conectados: {grafo.componentes_conectados()}")
    print(f"Grau mínimo: {grafo.grau_minimo()}")
    print(f"Grau máximo: {grafo.grau_maximo()}")
    print(f"Caminho médio: {grafo.caminho_medio():.2f}")
    print(f"Diâmetro: {grafo.diametro()}")

    print("\nINTERMEDIAÇÃO DOS VÉRTICES:")
    intermediacoes = grafo.intermediacao()
    for v, valor in sorted(intermediacoes.items()):
        print(f"Vértice {v}: {valor:.2f}")

   
    exportar_estatisticas_para_csv(grafo, "estatisticas_grafo.csv")
